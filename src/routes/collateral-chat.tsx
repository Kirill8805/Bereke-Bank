...
import { startCollateralChat, sendCollateralManagerMessage, clearCollateralChat } from "@/lib/collateral-chat.functions";

export const Route = createFileRoute("/collateral-chat")({
  head: () => ({ meta: [{ title: "Тренажёр: Залоги — Bereke AI" }] }),
  component: CollateralChatPage,
});
...
function CollateralChatPage() {
  ...
  const startFn = useServerFn(startCollateralChat);
  const sendFn = useServerFn(sendCollateralManagerMessage);
  const clearFn = useServerFn(clearCollateralChat);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-20 bg-background/90 backdrop-blur border-b">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h1 className="font-bold text-lg">Тренажёр: Залоги</h1>
