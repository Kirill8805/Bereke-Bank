...
import { startChat, sendManagerMessage, clearChat } from "@/lib/chat.functions";

export const Route = createFileRoute("/chat")({
  head: () => ({ meta: [{ title: "Чат с ИИ — Bereke AI" }] }),
  component: ChatPage,
});
...
function ChatPage() {
  ...
  const startFn = useServerFn(startChat);
  const sendFn = useServerFn(sendManagerMessage);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-20 bg-background/90 backdrop-blur border-b">
        <div className="flex items-center justify-between px-4 h-14">
          <button onClick={() => navigate({ to: "/" })} className="tap text-2xl">←</button>
          <h1 className="font-bold text-lg">Чат бот</h1>
