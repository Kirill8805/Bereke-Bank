...
import { Loader2, X, ArrowLeft, FileDown } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/hooks/use-auth";
import { startProductsTest, submitProductsTest, clearProductsTest } from "@/lib/products-test.functions";
import matritsaAsset from "@/assets/matritsa.xlsx.asset.json";

export const Route = createFileRoute("/products-test")({
  head: () => ({ meta: [{ title: "Тест по продуктам — Bereke AI" }] }),
...
function ProductsTestPage() {
  ...
  const startFn = useServerFn(startProductsTest);
  const submitFn = useServerFn(submitProductsTest);
  const clearFn = useServerFn(clearProductsTest);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b">
        <div className="flex items-center px-4 h-14">
          ...
          <h1 className="flex-1 text-center font-semibold text-base pr-6">Тест по продуктам</h1>
        </div>
      </header>
          ...
          <>
            <section>
              ...
              <ol className="space-y-2 text-[15px] text-muted-foreground leading-relaxed">
                {questions.map((q, i) => (<li key={i}>{i + 1}. {q}</li>))}
              </ol>
            </section>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center animate-in fade-in">
      <div className="bg-card w-full sm:max-w-md rounded-t-xl sm:rounded-lg p-5 max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom">
        ...
        <div className="mt-4 p-3 bg-muted/60 rounded-md">
          <p className="text-xs font-semibold mb-2">Изучите материал и попробуйте снова пройти тест</p>
          <a
            href={matritsaAsset.url}
            download="Матрица условий по продуктам.xlsx"
            className="tap flex items-center gap-2 bg-card border rounded-md px-3 py-2 text-sm font-medium hover:border-primary/40"
          >
            <FileDown className="size-4 text-primary" />
            <span>Матрица условий по продуктам.xlsx</span>
          </a>
        </div>
