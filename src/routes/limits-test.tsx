...
import { startLimitsTest, submitLimitsTest, clearLimitsTest } from "@/lib/limits-test.functions";

export const Route = createFileRoute("/limits-test")({
  head: () => ({ meta: [{ title: "Тест: Лимиты — Bereke AI" }] }),
  component: LimitsTestPage,
});

type Summary = { score: number; summary: string; strengths: string[]; weaknesses: string[] };

function LimitsTestPage() {
  ...
  const startFn = useServerFn(startLimitsTest);
  const submitFn = useServerFn(submitLimitsTest);
  const clearFn = useServerFn(clearLimitsTest);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b">
        <div className="flex items-center px-4 h-14">
          ...
          <h1 className="flex-1 text-center font-semibold text-base pr-6">Тест: Лимиты</h1>
        </div>
      </header>
