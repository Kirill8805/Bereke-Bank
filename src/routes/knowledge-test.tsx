...
import { startTest, submitTest, clearTest } from "@/lib/test.functions";

export const Route = createFileRoute("/knowledge-test")({
  head: () => ({ meta: [{ title: "Тест B-Bonus — Bereke AI" }] }),
...
function KnowledgeTestPage() {
  ...
  const clearFn = useServerFn(clearTest);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-20 bg-background/95 backdrop-blur border-b">
        <div className="flex items-center px-4 h-14">
          ...
          <h1 className="flex-1 text-center font-semibold text-base pr-6">Тест: B-Bonus</h1>
        </div>
      </header>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center animate-in fade-in">
      <div className="bg-card w-full sm:max-w-md rounded-t-xl sm:rounded-lg p-5 max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom">
        <div className="flex justify-between items-start mb-3">
          ...
          <button onClick={onClose} className="tap p-1" aria-label="Закрыть">
            <X className="size-5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-5">
          <button onClick={onAgain} className="tap rounded-md border py-3 text-sm font-semibold">
            Ещё раз
          </button>
          <button onClick={onClose} className="tap rounded-md bg-primary text-primary-foreground py-3 text-sm font-semibold">
            В профиль
          </button>
        </div>
      </div>
