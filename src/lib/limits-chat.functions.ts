...
const LIMITS_QUESTIONS = [
  "Хочу снять 5 миллионов тенге сегодня в банкомате — почему не получается?",
  "Почему я не могу перевести по номеру карты сразу 5 миллионов тенге?",
  "Какие лимиты на перевод между моими счетами в приложении?",
  "Сколько максимум можно перевести в Россию через приложение Bereke?",
  "Хочу отправить деньги в Узбекистан на карту HUMO, какие лимиты?",
  "Можно ли увеличить суточный лимит на снятие наличных и на сколько?",
  "Какой годовой лимит на снятие наличных у клиента пакета First?",
  "Я снимаю деньги в банкомате Forte по карте Bereke — почему лимит маленький?",
  "Хочу отправить SWIFT-перевод 200 000 USD, можно ли?",
  "Сколько можно снять без карты через банкомат Bereke?",
  "Какие лимиты на перевод по номеру телефона?",
  "Можно ли через КЦ снять лимит по карте, и на какой срок?",
];

function pickQuestions(n: number): string[] {
  const shuffled = [...LIMITS_QUESTIONS].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, n);
}

function systemPrompt(questions: string[]) {
  return `Ты КЛИЕНТ банка Bereke Bank по имени ${BOT_NAME}. Обычный человек. Пользователь — МЕНЕДЖЕР банка, он отвечает ТЕБЕ. Тема консультации — ЛИМИТЫ по операциям, снятию наличных и переводам.

КРИТИЧЕСКИ ВАЖНО:
- Здороваешься ТОЛЬКО в самом первом сообщении. Дальше не здоровайся и не представляйся.
- НИКОГДА не объясняй лимиты сам. Ты не знаешь цифр — ты пришёл узнать.
- НИКОГДА не отвечай на свой же вопрос. Если менеджер ответил коротко/непонятно — переспроси как клиент: «а почему?», «а сколько именно?», «а что тогда делать?».
- Не называй точные суммы лимитов, тарифы — ты этого не знаешь.
...
function looksLikeEmployeeReply(text: string) {
  ...
  const employeePatterns = [
    ...
    /лимит составля/, /суточный лимит/, /максимальная сумма/,
    /200 000/, /4 200 000/, /20 000 000/, /100 000 usd/,
  ];
...
async function generateClientReply({
  ...
  try {
    ...
    console.error("[limits generateClientReply] AI error:", e?.message, e?.statusCode);
  ...
  const { text: repaired } = await generateText({
    ...
    system: systemPrompt(questions),
    prompt: `Менеджер ответил: "${lastManagerMessage}". Ты случайно начал говорить как сотрудник. Перепиши свою реплику как обычный клиент: не здоровайся, не объясняй термины, не называй суммы — лучше переспроси. 1–2 предложения.`,
  });
  return repaired.trim();
}

export const startLimitsChat = createServerFn({ method: "POST" })
...
export const sendLimitsManagerMessage = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    if (managerTurns >= MAX_TURNS) {
      ...
      const { text: rawEval } = await generateText({
        model,
        system: `Ты строгий тренер для банковских менеджеров Bereke Bank по теме "Лимиты". Оцени, насколько менеджер правильно назвал лимиты и условия. Отвечай ТОЛЬКО валидным JSON.`,
...
export const clearLimitsChat = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
